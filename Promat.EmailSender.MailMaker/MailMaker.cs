...
public partial class MailMaker : IMailMaker
{
    ...
    private MailMaker(IMailConfigurator mailConfigurator, ILogger logger)
    {
        mailConfigurator.SetMailMaker(this);
        _mailConfigurator = mailConfigurator;
        _logger = logger;
    }
    private MailMaker(IMailConfigurator mailConfigurator, ILogger logger, IEmailSender emailSender)
    {
        mailConfigurator.SetMailMaker(this);
        _mailConfigurator = mailConfigurator;
    ...
    public static MailMaker New(ILogger logger) => new(new MailConfigurator(), logger);
    public static MailMaker New(ILogger logger, IEmailSender emailSender) => new(new MailConfigurator(), logger, emailSender);
    public static MailMaker New(IEmailSender emailSender) => new(new MailConfigurator(), null, emailSender);
    public static MailMaker New(IMailConfigurator configurator) => new(configurator);
    public static MailMaker New(IMailConfigurator configurator, ILogger logger) => new(configurator, logger);
    public static MailMaker New(IMailConfigurator configurator, ILogger logger, IEmailSender emailSender) => new(configurator, logger, emailSender);
    public static MailMaker New(IMailConfigurator configurator, IEmailSender emailSender) => new(configurator, null, emailSender);
    ...
    public string GetHtml()
    {
        ...
        for (var currentLine = 0; currentLine < _lineCount; currentLine++)
        {
            ...
            if (_lineWithImagen.SingleOrDefault(x => x.lineNumber == lineNumber) is var lineWithImagen && !string.IsNullOrWhiteSpace(lineWithImagen.urlImage) && !string.IsNullOrWhiteSpace(lineWithImagen.rightLine))
            {
                numberLine++;
            ...
            if (_lineWithImagenAndTwoColumns.SingleOrDefault(x => x.lineNumber == lineNumber) is var lineWithImagenAndTwoColumns && !string.IsNullOrWhiteSpace(lineWithImagenAndTwoColumns.urlImage) && !string.IsNullOrWhiteSpace(lineWithImagenAndTwoColumns.leftLine)
            ...
            if (_lineWithColumns.SingleOrDefault(x => x.lineNumber == lineNumber) is var lineWithColumns
    ...
    public Task SendMailAsync(string to, string subject, IEnumerable<string> cc = null, string fromEmail = null, string fromName = null)
    {
        ...
        {
            throw new InvalidOperationException($"No se ha establecido ninguna instancia de {typeof(IEmailSender).FullName}");
        }
