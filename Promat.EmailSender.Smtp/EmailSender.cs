...
namespace Promat.EmailSender.Smtp;
...
public class EmailSender : IEmailSender
{
    ...
    public EmailSender(ILogger<EmailSender> logger, SmtpClient smtpClient)
    {
        Initialize(logger, smtpClient: smtpClient);
        _disposeSmtpClient = false;
    }
    public EmailSender(string host, int port, string user, string password, bool tlsEnabled = false)
    {
        Initialize(null, host, user, password, port, tlsEnabled);
    }
    public EmailSender(string host, int port, string user, string password, bool tlsEnabled = false, string defaultFromEmail = null, string defaultFromName = null, bool ignoreRemoteCertificateChainErrors = false, bool ignoreRemoteCertificateNameMismatch = false, bool ignoreRemoteCertificateNotAvailable = false)
    {
    ...
    public EmailSender(ILogger<EmailSender> logger, string host, int port, string user, string password, bool tlsEnabled = false, string defaultFromEmail = null, string defaultFromName = null, bool ignoreRemoteCertificateChainErrors = false, bool ignoreRemoteCertificateNameMismatch = false, bool ignoreRemoteCertificateNotAvailable = false)
    {
    ...
    private void Initialize(ILogger<EmailSender> logger = null, string host = null, string user = null, string password = null, int port = -1, bool tlsEnabled = false, string fromEmail = null, string fromName = null, SmtpClient smtpClient = null, bool ignoreRemoteCertificateChainErrors = false, bool ignoreRemoteCertificateNameMismatch = false, bool ignoreRemoteCertificateNotAvailable = false)
    {
        _logger = logger;
        _logger?.LogTrace("Se ha creado una nueva instancia de {sender}", nameof(EmailSender));
        _host = host;
        _user = user;
        _password = password;
        _port = port;
            ...
            _port < 0)
        {
            ...
            {
                _logger?.LogError("Imposible inicializar {SmtpSender}. Es necesario proveer un {SmtpClient}, ó un Host, Puerto, User y Password", nameof(EmailSender), nameof(SmtpClient));
                throw new ArgumentException($"Imposible inicializar {nameof(EmailSender)}. Es necesario proveer un {nameof(SmtpClient)}, ó un Host, Puerto, User y Password");
            }
        }
        else
        {
